...
- (NSString *) hmacSha1Signature:(NSURLRequest *)request oauthParameters:(NSString *)oauthParameters
{
    ...
    if ([requestParameters rangeOfString:@"x_auth_mode"].location != NSNotFound &&
        [requestParameters rangeOfString:@"x_auth_password"].location != NSNotFound &&
        [requestParameters rangeOfString:@"x_auth_username"].location != NSNotFound) {
        
        NSString *requestParametersToEncode = [NSString stringWithFormat:@"&%@", requestParameters];
        NSString *encodedUrl = [NSString stringWithFormat:@"%@", AFPercentEscapedQueryStringPairMemberFromStringWithEncoding([requestParametersToEncode componentsSeparatedByString:@"?"][0], NSUTF8StringEncoding)];
        
        NSString *requestStringConcat = [NSString stringWithFormat:@"%@%@", requestString, encodedUrl];
        requestString = requestStringConcat;
    }
