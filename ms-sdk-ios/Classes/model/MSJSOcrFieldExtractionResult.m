        ...
        if([derived_dict isKindOfClass:[NSArray class]]) {
            ...
            if([(NSArray*)derived_dict count] > 0) {
                for (int i=0 ; i<[(NSArray*)derived_dict count] ; i++) {
                    ...
                    MSJSFieldExtractionResultDerivedData* d = [[MSJSFieldExtractionResultDerivedData alloc] initWithValues:dict];
                    [objs addObject:d];
                }
    ...
    if(_derived != nil){
        if([_derived isKindOfClass:[NSArray class]]){
            ...
            for( int i=0 ; i<[(NSArray*)_derived count] ; i++ ) {
				MSJSFieldExtractionResultDerivedData *derived = [[MSJSFieldExtractionResultDerivedData alloc]init];
