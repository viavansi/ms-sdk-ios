...
+(ApiClient *)sharedClientFromPool:(NSString *)baseUrl {
    ...
    if(_pool == nil) {
        ...
        [ApiClient configureCacheReachibilityForHost:baseUrl];
    }
    
    @synchronized(self) {
        ApiClient * client = [_pool objectForKey:baseUrl];
        if (client == nil) {
...
+(void) configureCacheReachibilityForHost:(NSString*)host {
    [[ApiClient sharedClientFromPool:host].reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
    ...
    [[ApiClient sharedClientFromPool:host].reachabilityManager startMonitoring];
}
