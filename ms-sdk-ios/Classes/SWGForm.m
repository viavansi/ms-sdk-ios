...
-(id) initWithValues:(NSDictionary*)dict
{
    ...
    if(self) {
        ...
        if(setting_dict != nil)
            _setting = [[SWGSetting alloc]initWithValues:setting_dict];
        ...
        if([values_dict isKindOfClass:[NSArray class]]) {
            ...
            if([(NSArray*)values_dict count] > 0) {
                for (NSDictionary* dict in (NSArray*)values_dict) {
                    SWGFormValue* d = [[SWGFormValue alloc] initWithValues:dict];
                    [objs addObject:d];
                }
        ...
        if([nestedValues_dict isKindOfClass:[NSArray class]]) {
            ...
            if([(NSArray*)nestedValues_dict count] > 0) {
                for (NSDictionary* dict in (NSArray*)nestedValues_dict) {
                    SWGFormNestedValue* d = [[SWGFormNestedValue alloc] initWithValues:dict];
                    [objs addObject:d];
                }
...
-(NSDictionary*) asDictionary {
    ...
    if(_code != nil) dict[@"code"] = _code ;
        ...
        if([_containers isKindOfClass:[NSArray class]]){
            ...
            for( SWGContainer *containers in (NSArray*)_containers) {
    ...
    if(_setting != nil){
        if([_setting isKindOfClass:[NSArray class]]){
            NSMutableArray * array = [[NSMutableArray alloc] init];
            for( SWGSetting *setting in (NSArray*)_setting) {
    ...
    if(_values != nil){
        if([_values isKindOfClass:[NSArray class]]){
            NSMutableArray * array = [[NSMutableArray alloc] init];
            for( SWGFormValue *values in (NSArray*)_values) {
    ...
    if(_nestedValues != nil){
        if([_nestedValues isKindOfClass:[NSArray class]]){
            NSMutableArray * array = [[NSMutableArray alloc] init];
            for( SWGFormNestedValue *nestedValues in (NSArray*)_nestedValues) {
